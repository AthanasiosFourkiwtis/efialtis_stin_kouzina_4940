...
from src.io_utils import load_data, write_submission, PRED_DIR, MODEL_DIR
...
INNER_OOF_SPLITS = 5
TUNE_FRACTION = 0.2
WEIGHT_GRID = np.linspace(0.0, 1.0, 11)   # 0.0 = MiniLM only ... 1.0 = TF-IDF only
...
def full_proba(model, X, master_classes):
    ...
    if dec.ndim == 1:  # binary case: 1-D -> [-dec, dec] for classes_[0], classes_[1]
...
def run_fold(df_tr, df_va, emb_tr, emb_va, haz_classes, prod_classes, seed):
    ...
    word, char, x_fit = fit_tfidf(build_text(df_fit, include_metadata=True))
    x_tune = apply_tfidf(word, char, build_text(df_tune, include_metadata=True))
    x_va = apply_tfidf(word, char, build_text(df_va, include_metadata=True))

    hazA = train_svc(x_fit, y_haz_fit, C_HAZARD)
    ...
    prodA = train_svc(x_fit_p, y_prod_fit, C_PRODUCT)
    ...
    hazB = train_svc(emb_fit, y_haz_fit, C_HAZARD)
    prodB = train_svc(emb_fit, y_prod_fit, C_PRODUCT)
    ...
    for wh in WEIGHT_GRID:
        ...
        for wp in WEIGHT_GRID:
            ...
            s = score_st1(y_haz_tune, haz_pred, y_prod_tune, prod_pred)
            if s > best[0]:
                best = (s, wh, wp)
    _, wh, wp = best
    ...
    parts_ens = score_st1(y_haz_va, haz_pred_ens, y_prod_va, prod_pred_ens,
...
def write_log(s_A, s_ens, n_splits):
...
def submission_path(df, test, emb_pooled, emb_test, haz_classes, prod_classes):
    ...
    def build_bases(rows_df, rows_emb):
        ...
        word, char, x = fit_tfidf(build_text(rows_df, include_metadata=True))
        hazA = train_svc(x, y_h, C_HAZARD)
        ...
        prodA = train_svc(x_p, y_p, C_PRODUCT)
        hazB = train_svc(rows_emb, y_h, C_HAZARD)
        prodB = train_svc(rows_emb, y_p, C_PRODUCT)
    ...
    def predict_probs(bases, rows_df, rows_emb):
        ...
        x = apply_tfidf(word, char, build_text(rows_df, include_metadata=True))
    ...
    for wh in WEIGHT_GRID:
        ...
        for wp in WEIGHT_GRID:
            ...
            s = score_st1(y_haz_t, haz_pred, y_prod_t, prod_pred)
            if s > best[0]:
                best = (s, wh, wp)
    _, wh, wp = best
    ...
    out = write_submission(test, haz_pred, prod_pred, PRED_DIR / "submission_stacking.csv")
...
def main():
    ...
    parser.add_argument("--submit", action="store_true", help="also write submission_stacking.csv")
    ...
    print(f"\n[cv] {args.splits}-fold evaluation (A alone vs ensemble)...")
    ...
    sA, sE = summarize(A_parts), summarize(ens_parts)
    print("\n=== RESULT ===")
    print(f"  base A alone (Efialtis Stin Kouzina): ST1 = {sA['st1'][0]:.4f} +/- {sA['st1'][1]:.4f}")
    ...
    if gain > pooled_std:
        print("  -> solid gain (over 1 std). Worth a submission.")
    elif gain > 0:
        print("  -> Small gain within the noise. Borderline - try --splits 10.")
    else:
        print("  -> The ensemble does not help. Keep plain Efialtis Stin Kouzina.")
    write_log(sA, sE, args.splits)
    print("  (logged to results/cv_eval_log.csv)")
